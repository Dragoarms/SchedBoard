...
from ui import (
    render_header,
    render_sidebars,
    render_departure_card,
    check_and_play_alerts,
...
def render_dashboard():
    ...
    overdue_deps = (
        active_departures[active_departures["is_overdue"] == True]
    ...
    if st.session_state.get("show_map", False):
        ...
        if active_departures.empty:
            ...
            with col3:
                st.metric(
                    get_text("no_gps", lang),
                    len(active_departures) - len(with_locations),
                )
    ...
    if active_departures.empty:
        ...
        for group_id, group_deps in groups_dict.items():
            ...
            group_info = (
                groups_df[groups_df["id"] == int(group_id)].iloc[0]
