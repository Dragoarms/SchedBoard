...
def render_arrivals():
    ...
    st.markdown(
        f'<p class="main-header">{get_text("app_title", lang)} - {get_text("arrivals", lang)}</p>',
        unsafe_allow_html=True,
    )
    st.markdown(
        f'<p class="sub-header">{get_text("mark_returned", lang)}</p>',
        unsafe_allow_html=True,
    )
        ...
        for _, dep in overdue.iterrows():
            ...
            with col2:
                st.caption(
                    get_text(
                        "expected_at",
                        lang,
                        time=format_time(pd.to_datetime(dep["expected_return"])),
                    )
                )
                st.caption(
                    get_text(
                        "departed_at",
                        lang,
                        time=format_time(pd.to_datetime(dep["departed_at"])),
                    )
                )
            ...
            with col3:
                if st.button(
                    ...
                    key=f"return_{dep['id']}",
                    type="primary",
                ):
                    if mark_returned(dep["id"]):
                        st.success(
                            f"{dep['person_name']} {get_text('marked_returned', lang)}"
                        )
                        time.sleep(1)
                        st.rerun()
        ...
        for _, dep in on_time.iterrows():
            ...
            with col1:
                ...
                if dep["time_remaining"] > 0:
                    ...
                    st.caption(
                        get_text("time_remaining", lang, hours=hours, minutes=minutes)
                    )
                else:
...
def render_group_returns(active_departures, lang):
    ...
    groups_out = active_departures[active_departures["group_id"] != ""].groupby(
        "group_id"
    )
        ...
        for group_id, group_deps in groups_out:
            if not groups_df.empty:
                ...
                if not group_info.empty:
                    ...
                    st.markdown(
                        f"**{group_name}** ({get_text('members_out', lang, count=len(group_deps))})"
                    )
                    ...
                    if st.button(
                        get_text("mark_group_returned", lang),
                        key=f"group_return_{group_id}",
                        type="primary",
                    ):
                        if mark_group_returned(group_id):
                            st.success(
                                get_text(
                                    "group_returned_success", lang, name=group_name
                                )
                            )
                            time.sleep(1)
                            st.rerun()
...
def render_extensions(active_departures, lang):
    ...
    if selected_person:
        person_data = active_departures[
            active_departures["person_name"] == selected_person
        ].iloc[0]

        st.write(
            f"**{get_text('current_expected_return', lang)}:** {format_time(pd.to_datetime(person_data['expected_return']))}"
        )
        ...
        with col1:
            hours_to_extend = st.number_input(
                get_text("hours_to_extend", lang), min_value=1, max_value=24, value=1
            )
        ...
        with col2:
            ...
            gps_data = None
            if (
                "auto_gps_lat" in st.session_state
                and "auto_gps_lon" in st.session_state
            ):
                gps_data = {
                ...
                if location_to_store:
                    st.info(
                        get_text(
                            "location_recorded",
                            lang,
                            lat=location_to_store["lat"],
                            lon=location_to_store["lon"],
                        )
                    )
