...
def load_kmz_tracks():
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            return [], {
                "min_lat": DEFAULT_MAP_CENTER[0],
                "max_lat": DEFAULT_MAP_CENTER[0],
                "min_lon": DEFAULT_MAP_CENTER[1],
                "max_lon": DEFAULT_MAP_CENTER[1],
            }
                    ...
                    if kml_filename:
                        with kmz.open(kml_filename) as kml_file:
                            ...
                            for placemark in root.findall(".//kml:Placemark", ns):
                                ...
                                name = (
                                    name_elem.text if name_elem is not None else "Track"
                                )
                                ...
                                # Look for Points
                                for point in placemark.findall(
...
def get_automatic_gps():
    """Get GPS location automatically using JavaScript"""
    gps_component = components.declare_component(
        "gps_component", url="http://localhost:3001"  # This won't be used in production
    )

    # JavaScript to get location automatically
    gps_script = """
    <script>
    (function() {
...
def create_personnel_map(active_departures, lang="en"):
    ...
    # Always use KMZ bounds if available, with padding
    if bounds["min_lat"] != 90 and bounds["max_lat"] != -90:  # If we have valid bounds
    ...
    for _, dep in active_departures.iterrows():
        ...
        if "last_location" in dep and dep.get("last_location"):
            try:
                # Parse location data
                if (
                    isinstance(dep["last_location"], str)
                    and dep["last_location"].strip()
                ):
                    ...
                    if dep["is_overdue"]:
                        ...
                        time_text = get_text(
                            "time_remaining", lang, hours=hours, minutes=minutes
                        )
...
def simple_gps_button(label=None, key="gps"):
    """Deprecated - use automatic GPS instead"""
    return None
...
def get_gps_location_button():
    ...
    if st.button("📍 Get GPS Location", help="Click to share your current location"):
        ...
        st.info(
            "Please enable location services and allow this site to access your location."
        )
...
def validate_gps_data(gps_data):
    ...
    try:
        ...
        return {"lat": lat, "lon": lon, "timestamp": get_current_time().isoformat()}
    except:
        return None
