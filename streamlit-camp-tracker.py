...
# Suppress pandas warnings about duplicate labels
...
def init_db():
    ...
    # Check if we need to migrate (remove is_overdue column if it exists)
    try:
        c.execute("PRAGMA table_info(departures)")
        columns = c.fetchall()
        if any(col[1] == 'is_overdue' for col in columns):
            # Migration needed - recreate table without is_overdue
            c.execute("ALTER TABLE departures RENAME TO departures_old")
            c.execute('''
            ...
            c.execute('''
                ...
                SELECT id, person_name, destination, departed_at, expected_return, 
                       actual_return, phone, supervisor, company, extensions_count
                FROM departures_old
            ''')
            c.execute("DROP TABLE departures_old")
    except:
        pass
...
def get_active_departures():
    ...
    conn = sqlite3.connect(DB_PATH)
    try:
        # Select columns explicitly to avoid duplicates
        query = '''
            SELECT id, person_name, destination, departed_at, expected_return, 
                   actual_return, phone, supervisor, company, extensions_count,
                   CASE 
        ...
        # Ensure is_overdue is numeric
        if not df.empty and 'is_overdue' in df.columns:
            df['is_overdue'] = pd.to_numeric(df['is_overdue'], errors='coerce').fillna(0).astype(int)
        
        return df
    except Exception as e:
        # If there's an error, return empty dataframe
        print(f"Error getting active departures: {e}")
        return pd.DataFrame()
    finally:
        conn.close()
...
def upload_manifest(df):
    ...
    try:
        ...
        for _, row in df.iterrows():
            if pd.notna(row.get('name', None)) and str(row['name']).strip():
                add_personnel(
                    str(row['name']).strip(),
                    str(row.get('phone', '')).strip() if pd.notna(row.get('phone')) else None,
                    str(row.get('supervisor', '')).strip() if pd.notna(row.get('supervisor')) else None,
                    str(row.get('supervisor_phone', '')).strip() if pd.notna(row.get('supervisor_phone')) else None,
                    str(row.get('company', '')).strip() if pd.notna(row.get('company')) else None
                )
                uploaded_count += 1
...
if page == "📝 Departure Form":
    ...
    with col2:
        ...
        with metric_col2:
            if not active_departures.empty and 'is_overdue' in active_departures.columns:
                overdue_count = int(active_departures['is_overdue'].sum())
            else:
                overdue_count = 0
        ...
        if uploaded_file is not None:
            try:
                ...
                if st.button("Upload to Manifest", type="primary"):
                    uploaded_count = upload_manifest(df)
                    st.success(f"✅ Uploaded {uploaded_count} records to manifest")
                    st.rerun()
            except Exception as e:
    ...
    with tab3:
        ...
        if not all_departures.empty:
            ...
            with col2:
                ...
                if not all_departures.empty and 'actual_return' in all_departures.columns:
                    all_departures['actual_return_dt'] = pd.to_datetime(all_departures['actual_return'])
            ...
            with col3:
                if not all_departures.empty and 'departed_at' in all_departures.columns:
                    all_departures['departed_at_dt'] = pd.to_datetime(all_departures['departed_at'])
                    total_departures_today = len(all_departures[
                        all_departures['departed_at_dt'] >= today_start
                    ])
                else:
            ...
            with col4:
                if not all_departures.empty and 'actual_return' in all_departures.columns:
                    returned = all_departures[all_departures['actual_return'].notna()].copy()
                    if not returned.empty:
                        returned['departed_at_dt'] = pd.to_datetime(returned['departed_at'])
                        returned['actual_return_dt'] = pd.to_datetime(returned['actual_return'])
                        returned['duration_hours'] = (returned['actual_return_dt'] - returned['departed_at_dt']).dt.total_seconds() / 3600
                        avg_duration = returned['duration_hours'].mean()
                    else:
                        avg_duration = None
                else:
                    avg_duration = None
