...
from config import (
    get_text, get_current_time, LOCAL_TIMEZONE, 
    DEFAULT_DEPARTURE_HOURS, LOGO_URL
)
from database import (
    get_personnel, get_groups, add_personnel, add_departure, 
    add_group, get_active_departures
)
...
def render_departures():
    ...
    with col2:
        st.markdown(f'<p class="main-header">{get_text("app_title", lang)} - {get_text("departures", lang)}</p>', 
                   unsafe_allow_html=True)
    
    st.markdown(f'<p class="sub-header">{get_text("log_departure", lang)}</p>', 
               unsafe_allow_html=True)
...
def render_individual_departure(personnel_df, lang):
    ...
    with col1:
        ...
        all_names = [get_text('add_new_person', lang)] + personnel_df['name'].tolist() if not personnel_df.empty else [get_text('add_new_person', lang)]
        
        selected_name = st.selectbox(
            get_text('name', lang),
            options=all_names,
            index=0,
            help=get_text('start_typing', lang) if lang == 'en' else "Commencez à taper pour filtrer"
        )
        ...
        st.info(f"📅 {get_text('departure_time', lang)}: {now_local.strftime('%I:%M %p')} → {get_text('expected_return', lang)}: {expected_arrival.strftime('%I:%M %p')}")
        
        # GPS location capture
        st.write(get_text('capture_location', lang))
        
        # Use the simple GPS button
        gps_data = simple_gps_button(key="departure")
        
        # Only show additional fields if they're missing from records
        if selected_name == get_text('add_new_person', lang) or (person_data is not None and not person_data.get('phone')):
            phone = st.text_input(get_text('phone', lang))
        else:
            phone = person_data.get('phone') if person_data is not None else None
        
        if selected_name == get_text('add_new_person', lang) or (person_data is not None and not person_data.get('supervisor')):
        ...
        if selected_name == get_text('add_new_person', lang) or (person_data is not None and not person_data.get('company')):
            company = st.text_input(get_text('company', lang))
        else:
            company = person_data.get('company') if person_data is not None else None
        
        # Submit button
        if st.button(get_text('log_departure', lang), type="primary", use_container_width=True):
            # Prepare GPS data
            location_data = None
            if gps_data:
                location_data = {
            ...
            if new_name:  # New person
                if new_name.strip() and destination.strip():
                    if add_personnel(new_name, phone, supervisor, None, company):
                        if add_departure(new_name, destination, expected_arrival, phone, 
                                       supervisor, company, initial_location=location_data):
                            st.success(get_text('success_departure', lang, name=new_name, destination=destination))
                            if location_data:
                ...
                if destination.strip():
                    if add_departure(selected_name, destination, expected_arrival,
                        ...
                        st.success(get_text('success_departure', lang, name=selected_name, destination=destination))
                        if location_data:
                            st.info(get_text('location_captured', lang))
                        time.sleep(1)
                        st.rerun()
...
def render_group_departure(personnel_df, groups_df, lang):
    ...
    if group_option == get_text('select_group', lang) and not groups_df.empty:
        selected_group = st.selectbox(get_text('select_group', lang), groups_df['group_name'].tolist())
        ...
        st.info(f"📅 {get_text('expected_return', lang)}: {expected_arrival.strftime('%I:%M %p')}")
        ...
        if st.button(get_text('log_group_departure', lang), type="primary"):
            if destination.strip():
                ...
                for member in members:
                    member = member.strip()
                    if not add_departure(member, destination, expected_arrival, group_id=group_id):
                        success = False
                
                if success:
                    st.success(get_text('group_departure_success', lang, name=selected_group, destination=destination))
                    time.sleep(1)
                    st.rerun()
            else:
                st.error(get_text('error_destination', lang))
        ...
        if not personnel_df.empty:
            selected_members = st.multiselect(
                ...
                help=get_text('select_multiple', lang) if lang == 'en' else "Sélectionnez plusieurs personnes"
            )
            
            # Option to add new members not in manifest
            with st.expander(get_text('add_new_members', lang)):
                new_members_input = st.text_area(
                    get_text('new_members', lang) + " " + get_text('one_per_line', lang), 
                    height=100
                )
        else:
            selected_members = []
        ...
        if 'new_members_input' in locals() and new_members_input:
            new_members = [m.strip() for m in new_members_input.split('\n') if m.strip()]
            all_members.extend(new_members)
        
        if all_members:
            responsible_person = st.selectbox(
                get_text('responsible_person', lang),
                options=all_members
            )
        else:
        ...
        if st.button(get_text('create_group', lang), type="secondary"):
            if group_name and all_members and responsible_person:
                ...
                if group_id:
                    st.success(get_text('group_created', lang, name=group_name, count=len(all_members)))
                    time.sleep(1)
                    st.rerun()
            else:
