...
from database import (
    get_personnel, get_all_departures, get_active_departures, 
    get_groups, safe_dataframe_for_display, update_group,
    mark_returned, extend_departure
)
...
def render_management():
    ...
    st.markdown(f'<p class="main-header">{get_text("app_title", lang)} - {get_text("management", lang)}</p>', 
               unsafe_allow_html=True)
    st.markdown(f'<p class="sub-header">{get_text("monitor_manage", lang)}</p>', 
               unsafe_allow_html=True)
...
def render_active_departures_tab(lang):
    ...
    if active_departures.empty:
        ...
        action = st.selectbox(
            ...
            [get_text('view_only', lang), get_text('mark_returned', lang), get_text('extend_time', lang)]
        )
        ...
        display_df['Status'] = display_df.apply(
            lambda row: '🔴 ' + get_text('overdue', lang) if row['is_overdue'] 
            else '🟡 ' + get_text('due_soon', lang) if row['time_remaining'] < 0.5
            else '🟢 ' + get_text('on_time', lang), 
            axis=1
        )
        
        # Format time columns
        display_df['Departed'] = pd.to_datetime(display_df['departed_at']).dt.strftime('%I:%M %p')
        display_df['Expected'] = pd.to_datetime(display_df['expected_return']).dt.strftime('%I:%M %p')
        display_df['Time Remaining'] = display_df['time_remaining'].apply(
            lambda x: f"{int(x)}h {int((x % 1) * 60)}m" if x > 0 else f"Overdue {abs(int(x))}h"
        )
        
        # Select columns to display
        columns_to_show = ['Status', 'person_name', 'destination', 'Departed', 
                          'Expected', 'Time Remaining', 'phone', 'supervisor']
        ...
        if action == get_text('mark_returned', lang):
            ...
            if st.button(get_text('mark_selected_returned', lang), type="primary"):
                ...
                st.success(f"{len(selected_indices)} {get_text('marked_returned', lang)}")
                st.rerun()
            ...
            if selected_person_idx is not None:
                ...
                if st.button(get_text('extend_selected', lang), type="primary"):
                    ...
                    st.success(get_text('extended_success', lang, 
...
def render_map_view_tab(lang):
    ...
    if active_departures.empty:
        ...
        if show_no_gps:
            ...
            if not no_gps.empty:
                st.dataframe(
                    safe_dataframe_for_display(no_gps[['person_name', 'destination', 'phone']]),
...
def render_personnel_manifest_tab(lang):
    ...
    if personnel_df.empty:
        ...
        if search_term:
            mask = personnel_df['name'].str.contains(search_term, case=False, na=False) | \
                   personnel_df['company'].str.contains(search_term, case=False, na=False) | \
                   personnel_df['supervisor'].str.contains(search_term, case=False, na=False)
        ...
        # Display dataframe
        st.dataframe(
            safe_dataframe_for_display(filtered_df),
            use_container_width=True
        )
...
def render_groups_tab(lang):
    ...
    if groups_df.empty:
        ...
        selected_group_name = st.selectbox(
            get_text('select_group', lang),
            groups_df['group_name'].tolist()
        )
        
        if selected_group_name:
            group_data = groups_df[groups_df['group_name'] == selected_group_name].iloc[0]
            members = [m.strip() for m in group_data['members'].split(',')]
            
            st.info(f"**{get_text('responsible_person', lang)}:** {group_data['responsible_person']}")
            ...
            with st.expander(get_text('edit_group', lang)):
                ...
                available_personnel = [p for p in personnel_df['name'].tolist() if p not in members]
                
                add_members = st.multiselect(
                    get_text('add_members', lang),
                    available_personnel
                )
                
                remove_members = st.multiselect(
                    get_text('remove_members', lang),
                    members
                )
                ...
                if st.button(get_text('update_group', lang), type="primary"):
                    # Update members list
                    updated_members = [m for m in members if m not in remove_members] + add_members
...
def render_statistics_tab(lang):
    ...
    if not all_departures.empty:
        ...
        if not completed_trips.empty:
            completed_trips['departed_at'] = pd.to_datetime(completed_trips['departed_at'])
            completed_trips['actual_return'] = pd.to_datetime(completed_trips['actual_return'])
            completed_trips['duration'] = (completed_trips['actual_return'] - completed_trips['departed_at']).dt.total_seconds() / 3600
            ...
            with col3:
                overdue_count = len(all_departures[all_departures['is_overdue'] == 'True'])
